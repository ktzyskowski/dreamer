...
def multi_categorical(logits: torch.Tensor, n_categoricals: int, n_classes: int) -> Distribution:
    ...
    Returns:
        dist (Distribution): multi-categorical distribution object.
    """
    # reintroduce (categorical, class) dimensions if flattened
...
def policy_distribution(logits: torch.Tensor, uniform_mix: float = 0.01) -> Distribution:
    ...
    Args:
        logits (*, n_actions): unnormalized action logits from the actor net.
        uniform_mix (float): fraction of probability mass assigned to the uniform distribution.
    Returns:
        dist (Distribution): action policy distribution.
    """
...
def mixin_uniform(probs: torch.Tensor, split=0.01, dim=-1) -> torch.Tensor:
    ...
    Args:
        ...
        split (float): fraction of probability mass assigned to uniform distribution.
        dim (int): probability dimension, defaults to last dimension.
    Returns:
        mixed (Tensor): the tensor of mixed probabilities.
    """
